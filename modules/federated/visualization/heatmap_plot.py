...
def _plot_heatmap(
    ...
    if legend:
        ...
        # Customize the color bar to be slimmer and closer to the main plot
        legend_kwargs["shrink"] = (
            1  # Adjust the shrink factor to make the color bar slimmer
        )
        legend_kwargs["aspect"] = (
            50  # Adjust the aspect ratio to make the color bar thinner
        )
        legend_kwargs["pad"] = (
            0.02  # Adjust the padding to bring the color bar closer to the plot
        )
        plot_kwargs["cmap"] = sns.mpl_palette(
            colormap, int(dataframe.max().max() + 1)
