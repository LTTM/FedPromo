...
def plot_label_distributions(
    ...
    Args:
        data (Union[Partitioner, pd.DataFrame]): The data to plot. Either a Partitioner object
            or a DataFrame with the data to plot.
        label_name (int): Column name identifying label based on which the plot will be created.
    ...
    # Check that either partitioner or dataframe is provided
    if isinstance(data, Partitioner):
        ...
        raise TypeError(
            f"Expected either a Partitioner object or a DataFrame, but got {type(data)}."
        )

    _validate_parameters(plot_type, size_unit, partition_id_axis)

    if size_unit == "absolute" and partitioner is not None:
    ...
    elif size_unit == "percent" and partitioner is not None:
