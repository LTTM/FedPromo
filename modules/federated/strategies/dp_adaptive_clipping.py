...
Slightly modified from the original implementation in Flower.
...
__all__ = ["DifferentialPrivacyServerSideAdaptiveClipping"]
...
class DifferentialPrivacyServerSideAdaptiveClipping(Strategy):
    ...
    def __init__(
        ...
        if self.module2train not in ["all", "main", "head"]:
            raise ValueError(
                "The module2train parameter must be one of 'all', 'main' or 'head'."
            )
        if (
    ...
    def aggregate_fit(
        ...
        for _, res in results:
            ...
            norm_bit = adaptive_clip_inputs_inplace(
                model_update, self.clipping_norm, self.module2train, self.n_head_layers
            )
